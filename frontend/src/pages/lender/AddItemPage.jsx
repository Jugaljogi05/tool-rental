...
const MAX_AI_PAYLOAD_BYTES = 8 * 1024 * 1024;
...
const loadImage = (src) =>
  new Promise((resolve, reject) => {
    ...
    image.onerror = () => reject(new Error("Unable to load image for AI processing."));
    image.src = src;
  });

const compressImageForAi = async (file, maxSize = 1024, quality = 0.72) => {
...
const estimatePayloadBytes = (payload) => new Blob([JSON.stringify(payload)]).size;
...
const AddItemPage = () => {
  ...
  const buildAiPayload = async () => {
    const selectedImages = getSelectedImages(form).slice(0, MAX_AI_IMAGE_COUNT);
    const images = await Promise.all(
      selectedImages.map((imageFile) => compressImageForAi(imageFile))
    );
  ...
  const autoGenerateDetails = async () => {
    ...
    try {
      ...
      if (estimatePayloadBytes(payload) > MAX_AI_PAYLOAD_BYTES) {
        setError("Selected images are too large for auto-generate. Try smaller files or fewer extras.");
        return;
      }
      ...
      setError(
        status === 413
          ? "Selected images are too large for auto-generate. Try fewer or smaller photos."
          : err.response?.data?.message || "Unable to generate listing details."
      );
    } finally {
      ...
      >
        ...
        <div className="md:col-span-2 flex flex-wrap gap-3">
          <Button type="button" variant="ghost" onClick={autoGenerateDetails} disabled={isGenerating}>
            {isGenerating ? "Generating..." : "Auto-generate from photos"}
          </Button>
          <Button type="submit">List item</Button>
        </div>

        <p className="md:col-span-2 text-xs text-zinc-400">
          The generator reads your selected product photos locally, compresses them for speed, and
          then asks Gemini to prefill the listing fields.
        </p>
